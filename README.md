...
| `outputs/completeness.csv` | Узлы на границе и с неполным входящим потоком, а также предлагаемые запросы данных. |
| `outputs/routes.csv` | Наблюдаемые цепочки и циклы. |
| `outputs/sensitivity.csv` | Необязательный отдельный анализ изменения порогов; текущая команда `run_pipeline.py` его не создаёт. При отсутствии файла вкладка «Анализ» показывает пояснение. |
