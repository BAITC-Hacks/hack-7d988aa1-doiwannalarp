...
def gaps(enriched_df: pd.DataFrame) -> pd.DataFrame:
    mask = (enriched_df["role"] == "boundary") | (enriched_df["inflow_incomplete"])
    ...
    subset["reason"] = subset.apply(reason, axis=1)
    subset["request"] = "запросить исходящие переводы по узлу"
